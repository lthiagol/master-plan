...
**Interactive forks — prefer the asking tool.** At any discrete human-decision
fork in an interview (steps 2, 6–7) or any "how do you want to proceed?"
moment, prefer the harness's structured asking/question tool over printing
prose. Offer concrete options, mark at most one `(recommended)` — only when
you have evidence from context/conventions/risk; never fake it on a values
call — and always leave a "type your own" path. Open-ended clarifications stay
open-ended; do not force options where there is no discrete choice. If the
harness has no asking tool, fall back to numbered text options with the
...
4. mp milestone complete <id>   # → lifecycle: done; enters the review queue (NOT shipped)
5. mp validate
```
