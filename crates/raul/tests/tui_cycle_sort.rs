...
#[test]
fn o_cycles_id_title_priority_stage_created_updated_id_on_milestones() {
    let mut app = App::new();
    app.select_lane(Lane::Milestones);
    let r = MpRunner::new().expect("mp");

    // M205: cycle follows column order — Id → Title → Priority →
    // Stage → Created → Updated → Id (6 stops).
    ...
    apply_action(&mut app, &r, Action::CycleSortNext).unwrap();
    assert_eq!(app.lane_sort_key(Lane::Milestones), SortKey::Created);
    apply_action(&mut app, &r, Action::CycleSortNext).unwrap();
...
fn o_is_noop_on_overview_path_watch_settings() {
    let mut app = App::new();
    let r = MpRunner::new().expect("mp");
    for lane in [Lane::Overview, Lane::Path, Lane::Autopilot, Lane::Settings] {
