...
pub struct UiConfig {
    ...
    /// autopilot TUI surface (the agent side stays untouched —
    /// the `mp autopilot start` command is independent of this
    /// flag). Stored as `Option<bool>` so we can tell
    /// "never set" from "explicitly set to false"; the operator
    /// default is `false`.
    ///
    /// M229: renamed from `show_watch_tab`. The legacy key is no
