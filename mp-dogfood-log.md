...
## Entry 52 — 2026-09-04 — Entry 51's 4 doctor-touching CI flakes are environment-dependent, not flakes — fixed by stubbing herdr  <!-- points-at: M231 -->

- When:           2026-09-04, dogfood-51 triage pass. Reproduced each of entry 51's 4 test failures locally by stripping herdr from PATH (`env -i PATH='/tmp/no-herdr-bin:/opt/homebrew/bin:/usr/bin:/bin' cargo nextest run -p mp --no-fail-fast -E 'test(/<name>/)'`). Each test panicked on `doctor.status.success()` (or `doctor should pass after relocate`) at the exact line entry 51 predicted. Root cause confirmed: `mp doctor` / `mp doctor --project` gates `report.ok` on `herdr_cli_shape`, which flips to `ok=false` whenever `which herdr` returns `None`. CI runners ship without herdr installed; local macOS dev installs it via brew. The "flake" was an environment-specific false negative masquerading as a timing issue.
- Command:        `env -i PATH='/tmp/no-herdr-bin:/opt/homebrew/bin:/usr/bin:/bin' cargo nextest run -p mp --no-fail-fast -E 'test(/<name>/)' --no-fail-fast` for each of the 4 tests, plus `cargo nextest run -p mp --no-fail-fast -E 'test(/<name>/)'` (herdr-on-path) and `cargo nextest run -p mp --release --no-fail-fast -E 'test(/<name>/)'`.
- Observed:       All 4 tests fail in herdr-stripped env (4 distinct panic sites: config_load_reliability.rs:131, init_json.rs:55, p4_brownfield.rs:71, plan_relocate.rs:53 — same lines the CI log would surface). All 4 pass in herdr-on-path env (both debug and release). The fix:
  - `crates/mp/tests/common/fake_herdr.rs`: new `install_fake_herdr_for_doctor(env) -> String` helper that installs a minimal stub `herdr` at `<env.tmp>/fake-bin/herdr` and returns the modified PATH. The stub satisfies `which_herdr` + the `agent start --help` (must include `--kind` / `--pane`) and `pane split --help` (must be non-empty) shape probes the gate uses.
  - 4 test files updated to call `install_fake_herdr_for_doctor(&env)` and pass the PATH via `env.run_with_env(&[("PATH", &path)], args)` for the doctor call. `plan_relocate` switched the doctor call from `lib_api::run` (in-process) to `env.run_with_env` (subprocess) since `lib_api::run` doesn't expose a PATH hook; init + relocate still use lib_api since they don't need herdr.
- Suspected:      Same root cause for all 4 — `report.ok = report.ok && project_ok`, `project_ok = checks.iter().all(|c| c.ok)`, and the `herdr_cli_shape` check sets `ok = herdr_shape.compatible`. When herdr is missing, `which_herdr()` returns `None`, `compatible = false`, `ok = false`, `project_ok = false`, `report.ok = false`, `emit_and_exit_on_fail` returns `Err(ExitCode(1))`, subprocess exits 1. The 4 tests then panic on `out.status.success()` / `v["ok"].as_bool() == Some(true)`.
- Verdict:        **bug** — environment-dependent test contract, not a flake. The "wontfix" framing in entry 51 was wrong: the gate is not optional under `mp doctor --format json` (M197 F-11 / AC-02 explicitly made non-`ok` doctor exit non-zero so CI pipelines can detect it). The right fix is per-test isolation, which is what the stub provides. Same pattern as the original `real_sigint` flake (fixed in M228 by polling readiness instead of a fixed sleep) and the broader "CI-only failure" category — they're not flakes, they're environment assumptions baked into assertions.
- One-line:       4 doctor-touching tests fail in CI because `which herdr` returns None (no herdr installed on the runner) and the gate collapses `report.ok` to false; fix is per-test `install_fake_herdr_for_doctor` + `env.run_with_env(&[("PATH", &path)], ...)` so the subprocess sees a stub herdr.
- Status:         **bug** — fixed in 5 commits (18577fc helper + 4 per-test fixes: 94a79af / 4bd9b20 / 2e1d724 / b290049). All 4 tests now pass under both herdr-on-path and herdr-off-path, in debug and release, and under nextest `--profile ci` + `--stress-count 5`. No production code touched. Entry 51's `wontfix` verdict is superseded by this entry.
